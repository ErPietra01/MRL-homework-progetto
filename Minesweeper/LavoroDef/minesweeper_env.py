...
class MinesweeperEnv(object):
    ...
    def get_state_neighbors(self, coord):
        '''
        Come get_neighbors, ma legge i valori VISIBILI all'agente (self.state)
        invece della board gia' risolta (self.grid). Serve per capire se una
        mossa e' una scelta informata o un'ipotesi al buio ("guess"): usare
        self.grid qui sarebbe scorretto, perche' contiene anche le celle
        ancora nascoste con il loro vero valore.
        '''
        x, y = coord[0], coord[1]
        ...
        for col in range(y-1, y+2):
            for row in range(x-1, x+2):
                if ((x != row or y != col) and
                    ...
                    neighbors.append(self.state[idx]['value'])
    ...
    def step(self, action_index):
        ...
        # get neighbors before action (valori visibili all'agente, non la board risolta)
        neighbors = self.get_state_neighbors(coords)
